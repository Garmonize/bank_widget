...
def test_get_mask_card_number_with_fixtures(input_card_number_fix, result_card_number_fix):
...
def test_get_mask_card_number(input_card_number, expected_card_number):
...
def test_get_mask_card_number_error(wrong_card_number):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('76172634987162398476', '**8476')
    ]
)
def test_get_mask_account_number(input_account_number, expected_account_number):
...
def test_get_mask_account_number_error(wrong_account_number):
    with pytest.raises(TypeError) as exc_account_info:
        get_mask_account_number(wrong_account_number)

    assert str(exc_account_info.value) == 'Введите корректный номер счета. Номер должен быть состоять из 20 цифр, без пробелов'
